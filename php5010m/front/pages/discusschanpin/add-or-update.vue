	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discusschanpin`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discusschanpin`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discusschanpin`, this.ruleForm);
						}else{
							await this.$api.add(`discusschanpin`, this.ruleForm);
						}
						uni.setStorageSync('discusschanpinCleanType',true);
