...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscusschanpinCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscusschanpinCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discusschanpin');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discusschanpin');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('discusschanpin', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscusschanpin', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscusschanpin/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('discusschanpin', this.detail);
				this.$utils.msg('审核成功');
