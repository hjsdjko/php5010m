	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discusshuodongzixun`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discusshuodongzixun`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discusshuodongzixun`, this.ruleForm);
						}else{
							await this.$api.add(`discusshuodongzixun`, this.ruleForm);
						}
						uni.setStorageSync('discusshuodongzixunCleanType',true);
