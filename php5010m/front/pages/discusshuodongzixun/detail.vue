...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscusshuodongzixunCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscusshuodongzixunCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discusshuodongzixun');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discusshuodongzixun');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('discusshuodongzixun', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscusshuodongzixun', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscusshuodongzixun/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('discusshuodongzixun', this.detail);
				this.$utils.msg('审核成功');
