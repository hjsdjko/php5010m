...
<script>
	export default {
		...
		async onShow(options) {
			...
			this.getStoreup();
			let cleanType = uni.getStorageSync('discusshuodongzixunCleanType')
			if(cleanType){
				uni.removeStorageSync('discusshuodongzixunCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','huodongzixun');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','huodongzixun');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('huodongzixun', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discusshuodongzixun', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discusshuodongzixun/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('huodongzixun', this.detail);
				this.$utils.msg('审核成功');
