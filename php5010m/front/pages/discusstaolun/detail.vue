...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscusstaolunCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscusstaolunCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discusstaolun');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discusstaolun');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('discusstaolun', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscusstaolun', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscusstaolun/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('discusstaolun', this.detail);
				this.$utils.msg('审核成功');
