	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discusstaolun`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discusstaolun`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discusstaolun`, this.ruleForm);
						}else{
							await this.$api.add(`discusstaolun`, this.ruleForm);
						}
						uni.setStorageSync('discusstaolunCleanType',true);
