...
class ChanpinController extends CommonController {
    ...
    public function page(){
        ...
        $startCount = ($page-1)*$limt;
                    $lists = "select * from `chanpin` ".$where." order by ".$sort." ".$order." limit ".$startCount.",".$limt;
    ...
    public function save(){
        ...
        if (!empty($tmpData)&& isset($tmpData)){
            $postData = json_decode($tmpData,true);
                ...
                $k = implode(',',$keyArr);
        $v = implode(',',$valArr);
                        $sql = "INSERT INTO `chanpin` (".$k.") VALUES (".$v.")";
    ...
    public function update(){
        ...
        $value = implode(',',$v);
         $sql = "UPDATE chanpin SET ".$value." where id = ".$id;
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
    /**
    ...
    public function delete(){
        ...
        $sql = "delete from chanpin WHERE id in({$str})";
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
    /**
    ...
    public function info($id=false){
        ...
        if (!empty($id)){
            ...
            $where = "`name` = ".$name;
                ...
                $this->andone('chanpin',$id,$column='clicknum',$num=1);
                        $clicktime = "update chanpin set clicktime = ".date('Y-m-d H:i:s',time())." where ".$where;
        table_sql($clicktime);
                $sql = "select * from `chanpin` where ".$where;
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
    ...
    public function value(){
        ...
        $sql = "SELECT ".$xColumnName.",sum(".$yColumnName.") total FROM chanpin group by ".$xColumnName;
        if(urldecode($request[6]) == '日') {
            $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y-%m-%d') ".$xColumnName.", sum(".$yColumnName.") total FROM chanpin GROUP BY DATE_FORMAT(".$xColumnName.", '%Y-%m-%d')";
        }
        if(urldecode($request[6]) == '月') {
            $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y-%m') ".$xColumnName.", sum(".$yColumnName.") total FROM chanpin GROUP BY DATE_FORMAT(".$xColumnName.", '%Y-%m')";
        }
        if(urldecode($request[6]) == '年') {
            $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y') ".$xColumnName.", sum(".$yColumnName.") total FROM chanpin GROUP BY DATE_FORMAT(".$xColumnName.", '%Y')";
        }
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
		...
		$limt = isset($_REQUEST['limit'])?$_REQUEST['limit']:"5";
				$sort = 'clicknum';
				$lists = "select * from `chanpin` order by ".$sort." desc limit 0,".$limt;
		$result = table_sql($lists);
		$arrayData = array();
		if ($result->num_rows > 0) {
			while ($datas = $result->fetch_assoc()){
