<?php
session_start();
class HuodongzixunController extends CommonController {
    ...
    public function page(){
			...
			if(in_array($k, $this->columData)){
                if ($val != ''){
                    $where.= " and ".$k." like '".$val."'";
                                        ...
                                        $sql = "select * from `huodongzixun` ".$where;
        $count = table_sql($sql);
        if ($count->num_rows < 1){
            $numberCount = 0;
        }else{
        ...
        $startCount = ($page-1)*$limt;
                    $lists = "select * from `huodongzixun` ".$where." order by ".$sort." ".$order." limit ".$startCount.",".$limt;
    ...
    public function save(){
        ...
        if (!empty($tmpData)&& isset($tmpData)){
            $postData = json_decode($tmpData,true);
                ...
                $k = implode(',',$keyArr);
        $v = implode(',',$valArr);
                        $sql = "INSERT INTO `huodongzixun` (".$k.") VALUES (".$v.")";
    ...
    public function update(){
        ...
        $value = implode(',',$v);
         $sql = "UPDATE huodongzixun SET ".$value." where id = ".$id;
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
    /**
    ...
    public function delete(){
        ...
        $sql = "delete from huodongzixun WHERE id in({$str})";
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
    /**
    ...
    public function info($id=false){
        ...
        if (!empty($id)){
            ...
            $where = "`name` = ".$name;
                        ...
                        $sql = "select * from `huodongzixun` where ".$where;
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
    ...
    public function value(){
        ...
        $sql = "SELECT ".$xColumnName.",sum(".$yColumnName.") total FROM huodongzixun group by ".$xColumnName;
        if(urldecode($request[6]) == '日') {
            $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y-%m-%d') ".$xColumnName.", sum(".$yColumnName.") total FROM huodongzixun GROUP BY DATE_FORMAT(".$xColumnName.", '%Y-%m-%d')";
        }
        if(urldecode($request[6]) == '月') {
            $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y-%m') ".$xColumnName.", sum(".$yColumnName.") total FROM huodongzixun GROUP BY DATE_FORMAT(".$xColumnName.", '%Y-%m')";
        }
        if(urldecode($request[6]) == '年') {
            $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y') ".$xColumnName.", sum(".$yColumnName.") total FROM huodongzixun GROUP BY DATE_FORMAT(".$xColumnName.", '%Y')";
        }
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
